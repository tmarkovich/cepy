...
#else
double fabs(), pow(), round(), gam(), log(), exp(), psi();
...
/* Psi function expansion, AMS55 #15.3.10, #15.3.11, #15.3.12 */
	...
	p = (a+d1) * (b+d1) * s / gam(e+2.0);	/* Poch for t=1 */
	t = 1.0;
	do
		{
...
nosum:
	p = gam(c);
	y1 *= gam(e) * p / (gam(a+d1) * gam(b+d1));

	y *= p / (gam(a+d2) * gam(b+d2));
	if( (aid & 1) != 0 )
		y = -y;
